...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/08 17:04:31 by jiqarbac          #+#    #+#             */
/*   Updated: 2021/02/25 17:35:53 by jiqarbac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	burningship(t_mlx mlx)
{
	...
	while (++mlx.i < W)
	{
		...
		while (++mlx.j < H)
		{
			...
			{
				tmp = mlx.zr;
				mlx.zr = fabs(mlx.zr * mlx.zr - mlx.zi * mlx.zi + mlx.cr);
				mlx.zi = fabs(2 * tmp * mlx.zi + mlx.ci);
				mlx.it++;
			}
